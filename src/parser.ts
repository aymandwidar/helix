 ...
 */
export function generatePrismaSchema(ast: HelixAST, dbProvider: string = 'sqlite'): string {
    const provider = dbProvider === 'postgres' || dbProvider === 'supabase' ? 'postgresql' : dbProvider;
    let schema = `// Generated by Helix v11.1
...
export function generateUIPage(view: HelixView, strand: HelixStrand, allStrands?: HelixStrand[], themeClasses?: { primaryButton: string; secondaryButton: string; card: string; text: string; textMuted: string; heading: string; badge: string; statusColors: { success: string; warning: string; info: string } }): string {
  ...
  const toggleItem = async (item: ${modelName}) => {
    ...
    // Filter out auto-managed fields (id, createdAt, updatedAt) — they're
    ...
    const renderedFields = userFields(strand)
        ...
        .join('\n');
    const interfaceBody = dedupeInterfaceLines(`  id: string;
${renderedFields}
${relatedInterfaceFields}
${hasManyInterfaceFields}
  createdAt: string;`);
    const interfaceBodySafe = dedupeInlineFieldList(interfaceBody);
